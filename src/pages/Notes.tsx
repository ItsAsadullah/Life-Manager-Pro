  ...
  return (
    <div className="space-y-6 pb-20">
      <div className="flex justify-between items-center">
        <div>
          <h2 className="text-2xl font-bold text-gray-900 dark:text-white">{t('notes')}</h2>
          <div className="flex space-x-4 mt-1">
            <button 
              onClick={() => setFilter('all')}
              className={`text-sm font-medium ${filter === 'all' ? 'text-indigo-600 dark:text-indigo-400 underline' : 'text-gray-500 dark:text-gray-400'}`}
            >
              {t('allNotes')}
            </button>
            <button 
              onClick={() => setFilter('archived')}
              className={`text-sm font-medium ${filter === 'archived' ? 'text-indigo-600 dark:text-indigo-400 underline' : 'text-gray-500 dark:text-gray-400'}`}
            >
              {t('archived')}
            </button>
          </div>
          ...
          >
            <div className="flex justify-between items-center mb-4">
              <div className="flex space-x-2">
                <button 
                  ...
                  className={`p-2 rounded-lg transition-colors ${noteType === 'text' ? 'bg-black/10 dark:bg-white/10' : 'hover:bg-black/5 dark:hover:bg-white/5'}`}
                  title={t('textNote')}
                >
                  <Type size={18} />
                </button>
                <button 
                  ...
                  className={`p-2 rounded-lg transition-colors ${noteType === 'checklist' ? 'bg-black/10 dark:bg-white/10' : 'hover:bg-black/5 dark:hover:bg-white/5'}`}
                  title={t('checklist')}
                >
                  <CheckSquare size={18} />
                </button>
                <div className="relative">
                  ...
                  {showColorPicker && (
                    <div className="absolute top-full left-0 mt-2 p-2 bg-white dark:bg-gray-800 rounded-xl shadow-2xl border border-gray-100 dark:border-gray-700 flex space-x-2 z-10">
                      {COLORS.map(color => (
                        <button
                          key={color.name}
                          onClick={() => {
                          ...
                          className={`w-6 h-6 rounded-full border ${color.bg} ${color.border} ${selectedColor.name === color.name ? 'ring-2 ring-indigo-500 ring-offset-2 dark:ring-offset-gray-800' : ''}`}
                        />
                      ))}
                    </div>
                  )}
                ...
                <button 
                  ...
                  className={`p-2 rounded-lg transition-colors ${isPinned ? 'bg-black/10 dark:bg-white/10 text-indigo-600 dark:text-indigo-400' : 'hover:bg-black/5 dark:hover:bg-white/5'}`}
                  title={t('pinNote')}
                >
            ...
            <div className="space-y-4">
              <input
                ...
                className="w-full bg-transparent text-xl font-bold placeholder:text-gray-400 dark:placeholder:text-gray-500 focus:outline-none text-gray-900 dark:text-white"
              />
              
              <div className="flex items-center space-x-2">
                <span className="text-xs font-bold text-gray-400 dark:text-gray-500 uppercase tracking-widest">{t('category')}:</span>
                <select
                  value={category}
              ...
              {noteType === 'text' ? (
                <div className="relative">
                  <textarea
                    ...
                    rows={5}
                    className="w-full bg-transparent placeholder:text-gray-400 dark:placeholder:text-gray-500 focus:outline-none resize-none text-gray-900 dark:text-white"
                  />
                  <button
                    onClick={toggleRecording}
                    className={`absolute bottom-0 right-0 p-3 rounded-full shadow-lg ${
                      isRecording ? 'bg-red-500 text-white animate-pulse' : 'bg-white dark:bg-gray-700 text-gray-600 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-600'
                    }`}
                ...
                <div className="space-y-2">
                  {checklistItems.map((item) => (
                    <div key={item.id} className="flex items-center space-x-2 group">
                      <button 
                        ...
                        className={`p-1 rounded transition-colors ${item.checked ? 'text-indigo-600 dark:text-indigo-400' : 'text-gray-400 dark:text-gray-500'}`}
                      >
                        {item.checked ? <CheckSquare size={18} /> : <div className="w-[18px] h-[18px] border-2 border-gray-300 dark:border-gray-600 rounded" />}
                      </button>
                      <input
                        type="text"
                        value={item.text}
                        ...
                        className={`flex-1 bg-transparent focus:outline-none ${item.checked ? 'line-through text-gray-400 dark:text-gray-500' : 'text-gray-900 dark:text-white'}`}
                        autoFocus={!item.text}
                      />
                      <button 
                        onClick={() => removeChecklistItem(item.id)}
                        className="p-1 text-gray-400 dark:text-gray-500 opacity-0 group-hover:opacity-100 hover:text-red-500 dark:hover:text-red-400 transition-all"
                      >
                        <X size={14} />
                      </button>
                    </div>
                  ...
                  <button 
                    ...
                    className="flex items-center text-sm text-gray-500 dark:text-gray-400 hover:text-indigo-600 dark:hover:text-indigo-400 transition-colors mt-2"
                  >
              ...
              <div className="flex justify-end pt-4">
                <button
                  ...
                  className="flex items-center px-6 py-2.5 bg-black dark:bg-white text-white dark:text-black rounded-xl hover:bg-gray-800 dark:hover:bg-gray-200 disabled:opacity-50 shadow-lg transition-all font-bold"
                >
        ...
        {filteredNotes.length === 0 && !isAdding && (
          <div className="text-center py-20 bg-white dark:bg-gray-800 rounded-3xl border border-dashed border-gray-200 dark:border-gray-700">
            <div className="w-16 h-16 bg-gray-50 dark:bg-gray-900 rounded-full flex items-center justify-center mx-auto mb-4">
              <StickyNote className="text-gray-300 dark:text-gray-600" size={32} />
            </div>
            <h3 className="text-lg font-bold text-gray-900 dark:text-white">{t('noNotesFound')}</h3>
            <p className="text-gray-500 dark:text-gray-400">{t('createFirstNote')}</p>
          </div>
        )}
      </div>
    </div>
    ...
    >
      <div className="flex justify-between items-start mb-3">
        ...
        <div className="flex space-x-1 opacity-0 group-hover:opacity-100 transition-opacity">
          <button 
            onClick={() => onPin(note)}
            className={`p-1.5 rounded-lg hover:bg-black/5 dark:hover:bg-white/5 ${note.isPinned ? 'text-indigo-600 dark:text-indigo-400' : 'text-gray-400 dark:text-gray-500'}`}
          >
            <Pin size={16} fill={note.isPinned ? 'currentColor' : 'none'} />
          </button>
          <button 
            onClick={() => onArchive(note)}
            className={`p-1.5 rounded-lg hover:bg-black/5 dark:hover:bg-white/5 ${note.isArchived ? 'text-indigo-600 dark:text-indigo-400' : 'text-gray-400 dark:text-gray-500'}`}
          >
            <Archive size={16} />
          </button>
          <button 
            onClick={() => onDelete(note.id)}
            className="p-1.5 rounded-lg hover:bg-red-50 dark:hover:bg-red-900/30 text-gray-400 dark:text-gray-500 hover:text-red-500 dark:hover:text-red-400"
          >
            <Trash2 size={16} />
          </button>
        </div>
      ...
      <div onClick={() => onEdit(note)} className="cursor-pointer">
        {note.type === 'checklist' ? (
            ...
            {note.checklist?.slice(0, 5).map((item: any) => (
              <div key={item.id} className="flex items-center space-x-2">
                <button 
                  ...
                  className={`flex-shrink-0 ${item.checked ? 'text-indigo-600 dark:text-indigo-400' : 'text-gray-300 dark:text-gray-600'}`}
                >
                  {item.checked ? <CheckSquare size={14} /> : <div className="w-[14px] h-[14px] border border-gray-400 dark:border-gray-500 rounded-sm" />}
                </button>
                <span className={`text-sm text-gray-700 dark:text-gray-300 truncate ${item.checked ? 'line-through text-gray-400 dark:text-gray-500' : ''}`}>
                  {item.text}
                </span>
              </div>
            ))}
          ...
          <p className="text-gray-600 dark:text-gray-400 text-sm whitespace-pre-wrap line-clamp-6 mb-4">{note.content}</p>
        )}
      </div>

      <div className="flex justify-between items-center pt-4 border-t border-black/5 dark:border-white/10">
        <span className="px-2 py-0.5 bg-black/5 dark:bg-white/10 text-gray-600 dark:text-gray-400 text-[10px] rounded-full font-bold uppercase tracking-wider">
