    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 py-8 px-4 sm:px-6 lg:px-8">
      <div className="max-w-3xl mx-auto">
        <div className="mb-6 flex items-center justify-between">
          <Link to="/" className="inline-flex items-center text-indigo-600 dark:text-indigo-400 hover:text-indigo-800 dark:hover:text-indigo-300 font-medium">
            <ArrowLeft size={20} className="mr-2" />
            {t('backToApp')}
          </Link>
          <div className="flex gap-2">
            <button onClick={handleDownloadImage} className="flex items-center px-4 py-2 bg-white dark:bg-gray-800 text-gray-700 dark:text-gray-300 rounded-lg border border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700 shadow-sm text-sm font-medium transition-colors">
        ...
        <div id="shared-memo-content" className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 overflow-hidden">
          ...
          <div className="p-8">
              ...
              <table className="w-full mb-8">
                <thead>
                  <tr className="border-b-2 border-gray-200 dark:border-gray-700">
                    <th className="py-3 text-left text-sm font-semibold text-gray-600 dark:text-gray-400 uppercase tracking-wider">{t('item')}</th>
                    <th className="py-3 text-right text-sm font-semibold text-gray-600 dark:text-gray-400 uppercase tracking-wider">{t('qty')}</th>
                    <th className="py-3 text-right text-sm font-semibold text-gray-600 dark:text-gray-400 uppercase tracking-wider">{t('price')}</th>
                    <th className="py-3 text-right text-sm font-semibold text-gray-600 dark:text-gray-400 uppercase tracking-wider">{t('total')}</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
                  {memo.items.map((item: any, idx: number) => (
                    <tr key={idx}>
                      <td className="py-4 text-gray-900 dark:text-white font-medium">{item.name}</td>
                      <td className="py-4 text-right text-gray-600 dark:text-gray-400">{item.quantity} {t(item.unit)}</td>
                      <td className="py-4 text-right text-gray-600 dark:text-gray-400">{currencySymbol}{item.unitPrice}</td>
                      <td className="py-4 text-right text-gray-900 dark:text-white font-bold">{currencySymbol}{item.total.toLocaleString()}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
            ...
            <div className="flex justify-between items-center bg-gray-50 dark:bg-gray-900 p-6 rounded-xl border border-gray-100 dark:border-gray-700">
              <span className="text-lg font-bold text-gray-600 dark:text-gray-400 uppercase tracking-wider">{t('grandTotal')}</span>
              <span className="text-3xl font-bold text-indigo-600 dark:text-indigo-400">{currencySymbol}{memo.totalAmount.toLocaleString()}</span>
            </div>
            ...
            <div className="mt-8 pt-8 border-t border-gray-100 dark:border-gray-700 text-center text-sm text-gray-400 dark:text-gray-500">
              <p className="font-bold text-gray-500 dark:text-gray-400 mb-1">{t('developedBy')}: Asadullah Al Galib</p>
